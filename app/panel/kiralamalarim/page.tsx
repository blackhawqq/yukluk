...
import { MessageSquare, X, Star, ShoppingBag } from "lucide-react";
...
export default function KiralamalarimPage() {
  ...
  useEffect(() => {
    fetch("/api/rentals/my?type=renter")
      .then(r => r.json())
      .then(({ rentals }) => { setRentals(rentals || []); setLoading(false); });
  }, []);

  const handleCancel = async (id: string) => {
    if (!confirm("Kiralamayı iptal etmek istediğinize emin misiniz?")) return;
    await fetch("/api/rentals/my", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ rentalId: id, status: "cancelled" }),
    });
    ...
    <div className="p-6 max-w-4xl mx-auto">
        ...
        {tabs.map(tab => {
          ...
          return (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              ...
              {tab.label}
              {count > 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? "bg-forest text-cream" : "bg-cream text-stone"}`}>
                  {count}
                </span>
              )}
            </button>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-cream-dark">
          ...
          <Link href="/ekipmanlar"><Button size="sm">Ekipman Bul</Button></Link>
        </div>
      ) : (
                    ...
                    <div className="flex gap-2">
                      {(rental.status === "confirmed" || rental.status === "active") && (
                        <Link href={`/panel/mesajlar?rental=${rental.id}`}>
                          <Button size="sm" variant="outline"><MessageSquare className="w-3.5 h-3.5" /> Mesaj</Button>
                        </Link>
                      )}
                      ...
                      {rental.status === "completed" && (
                        <Button size="sm" variant="outline"><Star className="w-3.5 h-3.5" /> Değerlendir</Button>
                      )}
                    </div>
                  </div>
                </div>

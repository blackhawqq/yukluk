...
export default function PanelPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [rentalsAsRenter, rentalsAsOwner] = await Promise.all([
        (supabase.from("rentals").select("*, equipment(*), owner:profiles!owner_id(*)").eq("renter_id", user.id).order("created_at", { ascending: false }).limit(3) as any),
        (supabase.from("rentals").select("*, equipment(*), renter:profiles!renter_id(*)").eq("owner_id", user.id) as any),
      ]);

      const ownerRentals: Array<{ status: string; owner_payout: number }> = rentalsAsOwner.data || [];
      const totalEarnings = ownerRentals.filter((r) => r.status === "completed").reduce((sum, r) => sum + (r.owner_payout || 0), 0);
      const activeCount = ownerRentals.filter((r) => r.status === "active").length;
      const pendingCount = ownerRentals.filter((r) => r.status === "pending").length;
      ...
      setRecentRentals((rentalsAsRenter.data as RentalWithDetails[]) || []);
      setLoading(false);
    };
    load();
  }, [user]);
  ...
  const statCards = [
    ...
    { label: "Ortalama Puan", value: stats.avgRating > 0 ? `${stats.avgRating.toFixed(1)} ★` : "—", icon: Star, color: "text-orange", bg: "bg-orange/10", isText: true },
  ];
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-cream-dark p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/panel/kiralamalarim" className="text-forest text-xs hover:underline">
              Tümünü gör
            </Link>
          </div>
          {loading ? (
